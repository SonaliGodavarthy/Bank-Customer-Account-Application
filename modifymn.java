...
/* This program is for modifying mobile number in bank application */

package project.modmn;
import java.awt.*;
...
public class modifymn
{
	//declaration of frame, buttons, textfield, panels and labels
	JFrame frame_modifymobilenumber;
	
	JPanel p_accountnumber,p_newmobilenumber,p_submitbtn;
	
	JTextField tf_accountnumber,tf_newmobilenumber;
	...
	public modifymn()
	{
		...
		//creation a frame
		frame_modifymobilenumber = new JFrame();
		
		//creation text field
		tf_accountnumber=new JTextField(35);
		tf_newmobilenumber=new JTextField(35);
		...
		p_newmobilenumber.setLayout(new GridLayout(1,1));
		p_newmobilenumber.add(new JLabel("Enter your new mobile number: "));
		p_newmobilenumber.add(tf_newmobilenumber);
		...
		p_newmobilenumber.setBounds(400,750,1000,20);
		p_submitbtn.setBounds(800,800,100,20);
		
		//adding panels to frame
		frame_modifymobilenumber.add(p_cal);
		frame_modifymobilenumber.add(p_accountnumber);
		frame_modifymobilenumber.add(p_newmobilenumber);
		frame_modifymobilenumber.add(p_submitbtn);
		frame_modifymobilenumber.add(p_dis_details);
		frame_modifymobilenumber.add(p_dis_account);
		frame_modifymobilenumber.add(p_dis_name);
		frame_modifymobilenumber.add(p_dis_fathersname);
		frame_modifymobilenumber.add(p_dis_mothersname);
		frame_modifymobilenumber.add(p_dis_gender);
		frame_modifymobilenumber.add(p_dis_dob);
		frame_modifymobilenumber.add(p_dis_mobilenumber);
		frame_modifymobilenumber.add(p_dis_emailid);
		frame_modifymobilenumber.add(p_dis_address);
		
		
		frame_modifymobilenumber.addWindowListener(new WindowAdapter(){
		  public void windowClosing(WindowEvent e) 
		  {
			frame_modifymobilenumber.dispose();                               //closing of frame
		  }
		});
		
		frame_modifymobilenumber.setExtendedState(frame_modifymobilenumber.getExtendedState() | frame_modifymobilenumber.MAXIMIZED_BOTH);    //for maximizing window
		...
		tf_accountnumber.addKeyListener(new KeyAdapter() 
		{
						...
						if(ke.getKeyChar()==10)
						{
							...
							if(tf_accountnumber.getText().length()==0)
							{
														...
														lbl_dis_fathersname.setText("");
														  ...
														  tf_newmobilenumber.setText("");
														  
														//display of dialog box
														JDialog dg_error=new JDialog(frame_modifymobilenumber,"Error",true);
									...
									String acc_status="";
										...
										ResultSet rs = statement.executeQuery(sql);
										  ...
										  while(rs.next())
										  {
											  ...
											  tf_newmobilenumber.requestFocus();
										  ...
										  if(count==0)
											{
															...
															JDialog dg_error=new JDialog(frame_modifymobilenumber,"Invalid account number",true);
											...
											else if(acc_status.equals("c"))
											{
												lbl_dis_accountnumber.setText("");
															...
															JDialog dg_accountclosed=new JDialog(frame_modifymobilenumber,"Account closed",true);
				...
				btn_submit.addActionListener(new ActionListener()
				{
					public void actionPerformed(ActionEvent ett) 
					{
						...
						if(tf_newmobilenumber.getText().length()==10 && tf_newmobilenumber.getText().length()!=0)
						{
							//dialog box to confirm the data
							JDialog dg_confirmation=new JDialog(frame_modifymobilenumber,"Confirmation",true);
						        	...
						        	{  
							        	    ...
							        	    {  
												try{
													...
													String cmn=tf_newmobilenumber.getText().toString();
													//upadateing data of the accountumber
														String sql1 = "UPDATE newaccount set mobilenumber="+cmn+" where accountnumber="+accno;
						...
						//if length of the mobile number is less than 10
						else if(tf_newmobilenumber.getText().length()<10 && tf_newmobilenumber.getText().length()!=0)
						{
							//dialog box for invalid mobile number
							JDialog dg_error=new JDialog(frame_modifymobilenumber,"Invalid mobile number",true);
								dg_error.add(new JLabel("Please enter valid mobile number"));
								ok = new JButton ("ok");
								dg_error.add(ok);
						...
						//if length of the mobile number is 0
						else if(tf_newmobilenumber.getText().length()==0)
						{
							JDialog dg_error=new JDialog(frame_modifymobilenumber,"Error",true);
								dg_error.add(new JLabel("Please enter mobile number"));
								ok = new JButton ("ok");
								dg_error.add(ok);
		...
		//adding key listener to mobile number so that it doesn't take alphabets and length is not less than 10
		tf_newmobilenumber.addKeyListener(new KeyAdapter() 
		{
         		public void keyPressed(KeyEvent ke) 
			{
				if(ke.getKeyChar()==10)
				{
					btn_submit.requestFocus();
            			...
            			int len_mobilenumber = tf_newmobilenumber.getText().length();
            			if (ke.getKeyChar() >= '0' && ke.getKeyChar() <= '9' )  
						{
               				tf_newmobilenumber.setEditable(true);
            			} 
						else if( ke.getKeyChar()==8)
						{
									tf_newmobilenumber.setEditable(true);
						...
						{
									tf_newmobilenumber.setEditable(false);
						}
		...
		//setting title to frame and making the frame visible	 
		 frame_modifymobilenumber.setLayout(null);
		 frame_modifymobilenumber.setTitle("Modify Mobile Number");
		frame_modifymobilenumber.setVisible(true);
